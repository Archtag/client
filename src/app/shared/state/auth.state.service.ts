...
export class AuthStateService {
	...
	authenticate$ = new Subject<{ user: IAuthStateUser, token: string, refreshToken: string }>();
	signOut$ = new Subject<void>();

	constructor() {
		this.authenticate$.pipe(takeUntilDestroyed()).subscribe(({ user, token, refreshToken }) => {
			this.state.set({
